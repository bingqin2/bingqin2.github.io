...
| `site/CoDraft/` | CoDraft's own website at `/CoDraft/`, standalone from the rest of the site: `index.html` and `codraft.css`, in the colors and type of the CoDraft application, with an Open CoDraft button that leads to the public demo at https://codraft-demo.onrender.com. Screenshots in `images/` are WebP, 1280 and 2560 px wide (390 and 780 px for the phone), picked through `srcset`, and `images/og.jpg` is the Open Graph preview. |
